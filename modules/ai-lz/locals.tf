locals {
  ...
  # dev -> Developer, qua -> Standard, prod -> Premium
  #apim_sku_root     = var.environment == "prod" ? "PremiumV2" : "StandardV2"
  apim_sku_root     = var.environment == "prod" ? "Premium" : "Premium"
  #apim_sku_capacity = var.environment == "prod" ? 2 : 1
  apim_sku_capacity = var.environment == "prod" ? 3 : 3
  ...
  bastion_zones = var.environment == "prod" ? ["1", "2", "3"] : var.environment == "qua" ? ["1"] : []
