...
@description('Address prefix for the Windows jumpbox subnet. Required for existing VNets when enableJumpbox is true.')
...
var bastionSubnetSpaces = empty(bastionSubnetPrefix) ? cidrSubnet(vnetAddressSpace, 26, 8) : bastionSubnetPrefix
var jumpboxSubnetSpaces = empty(jumpboxSubnetPrefix) ? cidrSubnet(vnetAddressSpace, 24, 2) : jumpboxSubnetPrefix
...
module bastionSubnet 'subnet.bicep' = if (enableJumpbox) {
  ...
  params: {
    ...
    addressPrefix: bastionSubnetSpaces
    delegations: []
  }
  dependsOn: [
    peSubnet
