...
@description('Short suffix used in resource names. Defaults to a stable value based on the resource group; set this explicitly to reuse an existing deployment suffix.')
@minLength(4)
@maxLength(4)
...
@description('Optional name of the Windows jumpbox VM. If empty, a name is generated from the Foundry account name.')
...
@description('Optional name of the Azure Bastion public IP. If empty, a name is generated from the Foundry account name.')
param bastionPublicIpName string = ''

@description('Optional name of the jumpbox network security group. If empty, a name is generated from the Foundry account name.')
param jumpboxNsgName string = ''

@description('Optional name of the jumpbox network interface. If empty, a name is generated from the Foundry account name.')
...
param jumpboxWindowsImageSku string = '2022-datacenter-azure-edition'
...
param jumpboxOsDiskSku string = 'StandardSSD_LRS'
...
@description('Create a new Application Insights resource for tracing, or connect an existing one.')
@allowed([
  'new'
  'existing'
])
...
@description('Full ARM Resource ID of an existing Application Insights resource for Foundry tracing. When provided, no new Application Insights or Log Analytics workspace is created.')
...
var resolvedJumpboxVmName = !empty(jumpboxVmName) ? jumpboxVmName : take('vm-${accountName}-jumpbox', 64)
var resolvedBastionName = !empty(bastionName) ? bastionName : take('bas-${accountName}', 80)
var resolvedBastionPublicIpName = !empty(bastionPublicIpName) ? bastionPublicIpName : take('pip-${accountName}-bastion', 80)
var resolvedJumpboxNsgName = !empty(jumpboxNsgName) ? jumpboxNsgName : take('nsg-${accountName}-jumpbox', 80)
var resolvedJumpboxNicName = !empty(jumpboxNicName) ? jumpboxNicName : take('nic-${accountName}-jumpbox', 80)
...
module windowsJumpbox 'modules-network-secured/windows-jumpbox.bicep' = if (enableJumpbox) {
  name: 'jumpbox-${accountName}-${uniqueSuffix}-deployment'
  scope: resourceGroup(vnetSubscriptionId, vnetResourceGroupName)
  params: {
    location: location
    jumpboxVmName: resolvedJumpboxVmName
...
module applicationInsights 'modules-network-secured/application-insights.bicep' = {
  name: 'appinsights-${accountName}-${uniqueSuffix}-deployment'
  params: {
    location: location
    accountName: aiAccount.outputs.accountName
    newOrExistingApplicationInsights: newOrExistingApplicationInsights
    applicationInsightsName: applicationInsightsName
    applicationInsightsResourceId: applicationInsightsResourceId
    logAnalyticsWorkspaceName: logAnalyticsWorkspaceName
    applicationInsightsRetentionInDays: applicationInsightsRetentionInDays
  }
}
...
output appInsightsConnectionName string = applicationInsights.outputs.appInsightsConnectionName
